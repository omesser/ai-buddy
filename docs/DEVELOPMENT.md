...
| `AI_BUDDY_DIRECTOR_BLANK` | Blank-AI mode: the opening turn carries the behavior list, the reply contract and what just happened, and nothing else — no Personality Prompt, no Instance Prompt, no voice rules. Off unless set, and Settings → Development's "Blank AI" says the same thing. It is the control run for telling a model's misbehaviour apart from the prompt's; a buddy in this mode has no character. The Harness lane keeps a session of its own for it, so no session mixes the two prompts. #657. |
