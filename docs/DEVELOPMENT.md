...
| `AI_BUDDY_DIRECTOR_BLANK` | Blank-AI mode: every turn carries what just happened and nothing else — no Personality Prompt, no Instance Prompt, no voice rules, no behavior list, no reply contract. Off unless set, and Settings → Development's "Blank AI" says the same thing. It is the control run for telling a model's misbehaviour apart from the prompt's. Nothing tells the model to name a Behavior, so the reply is prose and the buddy speaks it without acting: intended, and the whole point of a turn with no instruction in front of it. The Harness lane keeps a session of its own for the mode, so no session mixes the two prompts. #657. |
...
Settings → AI persists base URL, model, and the first wake interval, and stores the API key in the OS secret store (Keychain on macOS; Secret Service/keyutils on Linux). Settings → Development persists the Model API timeout and reply cap, blank-AI mode, and — under Harness — the turn timeout, the auth-retry interval, and the MCP server binary. Editing any of those retargets the running Director: the next wake reaches the new host on the new interval, and the session in flight is dropped rather than answered against the old one — a streaming call closes its connection, so the old host stops generating too. No restart.
