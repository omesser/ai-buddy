...
- **Ignores `mcpServers` altogether** → `cursor-agent` loads MCP servers only from an approved `.cursor/mcp.json`, and hands a stdio server nothing from its own environment (#1020). So the app also serves the same dispatch on a unix socket, `<data folder>/mcp.sock` at mode 0600 with no bearer token on that transport, and attach writes an entry naming only the shim and `--sock <path>` into `<cwd>/.cursor/mcp.json`, merged beside whatever servers the file already holds, then runs `cursor-agent mcp enable ai-buddy` before spawning `cursor-agent acp`. Nothing in that entry changes between launches, so Cursor's approval id (a hash of the cwd and the entry) is stable and `enable` is idempotent. Detach removes the entry, and the file and directory when attach created them. Inside a git worktree the file is named in `<gitdir>/info/exclude`, which is local and never committed. The socket is `cfg(unix)`; on Windows no entry is written and `cursor-agent` keeps its gap.
