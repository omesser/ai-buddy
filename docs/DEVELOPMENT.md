...
| `AI_BUDDY_DIRECTOR_MAX_TOKENS` | Reply cap for the HTTP Completer, spent on thinking as well as reply — the wire carries one number for both. Overrides Settings → Development's "Reply cap, in tokens", and when neither says anything it is 80 remote and 512 local, or 8192 against a host that has marked its thinking. A number either of them names wins over all three. A Harness decides its own reply length. #606. |
