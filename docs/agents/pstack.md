...
| Signing | pstack's pull request guidance carries no signature | `docs/agents/writing.md` ends every issue comment, pull request description, review and inline reply an agent writes with a line that names the Harness (`Grok Build agent`, `Cursor agent`) or, for a named Grok bot, `Grok bot (<Name>)` | This repository. The sign-off is additive; pstack does not remove it. Never put a Grok bot instance name in a Harness agent's parentheses. |
