...
| Code comments | pstack | `no-comments` and the `comment-sicko` agent delete comments, keeping only a *why* the code cannot show | `docs/agents/comments.md` governs. Do not run `/no-comments` here. `comment-sicko` is reachable through the symlink, not sanctioned. |
