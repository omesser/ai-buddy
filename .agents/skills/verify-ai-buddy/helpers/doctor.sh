...
if [ "$UNITS" = "1" ]; then
  ...
  if bash scripts/test_verify_overlay_stray_scope.sh > /dev/null; then
    pass "scripts/test_verify_overlay_stray_scope.sh"
  else
    fail "scripts/test_verify_overlay_stray_scope.sh"
  fi
