...
- **Linux X11 full check.** Run `xvfb-run -a -s "-screen 0 1280x720x24" .agents/skills/verify-ai-buddy/helpers/drive-overlay-x11.sh`. Exit code `0` confirms presence. When `xprop _NET_WM_STATE` is empty but the app log has `^overlay:`, `EWMH configured`, and `frame:` lines, the script WARNs and continues; WARN is not failure (see Gotchas). Prefer `frame:.*Perched` when a Perch window existed before launch; Falling/Grounded frames still count as presence.
