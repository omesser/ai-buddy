...
- `poke-resume` after the react animation (~600ms), the buddy returns to idle (Grounded) or sit (Perched); any interrupted walk does not resume (Director's fresh call determines the next motion).
