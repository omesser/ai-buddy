...
mod tests {
    ...
    #[test]
    fn test_apply_enabled_states_applies_to_all_controls() {
        unsafe {
            gtk::set_initialized();
        }

        let controls = Rc::new(RefCell::new(HashMap::new()));

        controls.borrow_mut().insert(
            form::DIRECTOR_BASE_URL_ID.to_string(),
            Control::Entry(gtk::Entry::new()),
        );
        controls.borrow_mut().insert(
        ...
        controls.borrow_mut().insert(
            form::DIRECTOR_API_KEY_ID.to_string(),
            Control::Entry(gtk::Entry::new()),
        );
        controls.borrow_mut().insert(
            form::APPLY_ID.to_string(),
            Control::Button(gtk::Button::new()),
        );
        controls.borrow_mut().insert(
            form::DIRECTOR_BASE_URL_PICK_ID.to_string(),
            Control::ComboBox(gtk::ComboBoxText::new(), vec![]),
        );

        let description = form::describe();

        for (id, control) in controls.borrow().iter() {
            let expected_frozen = row_frozen(&description, id).unwrap_or(false);
            ...
            match control {
                Control::Entry(entry) => {
                    ...
                    let actual_editable = entry.is_editable();
                    assert_eq!(
                        !expected_frozen, actual_editable,
                        "Entry '{}' editable state should match frozen state",
                        id
                    );
                }
                Control::Button(button) => {
                    ...
                    let actual_sensitive = button.is_sensitive();
                    assert_eq!(
                        !expected_frozen, actual_sensitive,
                        "Button '{}' sensitive state should match frozen state",
                        id
                    );
                }
                Control::ComboBox(combo, _) => {
                    ...
                    let actual_sensitive = combo.is_sensitive();
                    assert_eq!(
                        !expected_frozen, actual_sensitive,
                        "ComboBox '{}' sensitive state should match frozen state",
                        id
                    );
                }
                _ => {}
