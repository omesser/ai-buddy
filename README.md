...
| anything else | as typed, split on whitespace | Unnamed, and it works: any command that speaks ACP on stdio attaches. <img src="https://cdn.simpleicons.org/githubcopilot" width="14" alt="" /> GitHub Copilot CLI (`copilot --acp --stdio`) reaches ai-buddy this way today and earns a named row once smoked (#457). Goose (`goose acp`) is custom-argv-only until a smoked turn earns it a named preset (ADR-0022). Google has none: Antigravity (`agy`) speaks its own protocol rather than ACP, so it needs an adapter (#604). |
