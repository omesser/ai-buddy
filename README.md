...
13. **The two shipped Characters are two companions.** Run each in turn (see
    [The shipped Characters](#the-shipped-characters)) and watch it for a
    minute. BMO snaps between poses and is never off its feet; Nim eases
    between them and keeps ending up asleep. Then judge the drawing itself,
    which no test can: the frames are generated by a script and nobody has
    claimed they are good.

The last three need a second display, and only a window server can answer them:

14. **A Character on a seam is whole.** Drag the sprite slowly across the
...
15. **Either half can be clicked.** With the sprite straddling, click the half
    on each display in turn. Both pick it up, and clicking beside it on either
    display still reaches whatever is underneath.
16. **A display can come and go.** With the app running, unplug a display, or
...
- **`characters/modern/` — Nim**, modern pixel art. A palette larger than
  ...
  contact shadow, and twice the frames everywhere so the motion eases rather
  than steps. Nim comes to rest: every Behavior it declares but the walk ends
  sitting or asleep.

The difference is the Behaviors as much as the drawing, which is what makes
switching feel like a different companion rather than a reskin. Both are drawn
by a script rather than by hand, which is what the repository can honestly
claim; regenerate both with:
