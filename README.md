...
| `goose` | `goose acp` | First-party, Block. `goose` alone is the interactive CLI, so the subcommand is the whole of the row. Fresh and resumed sessions both work, smoked on goose 1.51.0. On a machine with no provider configured anywhere, a probe of goose 1.52.0 exits 2, because `session/new` returns Internal error until `goose configure` has run. An unset `GOOSE_PROVIDER` is not itself the trouble: Goose reads its provider from `~/.config/goose/config.yaml`, and the 1.51.0 smoke above ran with that variable unset. |
