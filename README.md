...
| `goose` | `goose acp` | First-party, Block. `goose` alone is the interactive CLI, so the subcommand is the whole of the row. Fresh and resumed sessions both work, smoked on goose 1.51.0. A probe of goose 1.52.0 with no provider exits 2: `session/new` returns Internal error because `GOOSE_PROVIDER` is unset (`goose configure`). |
...
| `goose` | Keeps shell and filesystem, has no web tool at all and no ask-user tool, and the other names it lists are Goose's own built-in extensions (`apps__`, `todo__`, `extensionmanager__`) rather than the user's servers. **ai-buddy's MCP does not reach the session.** `describe_screen` is absent from the eighteen tools Goose lists and the agent answers that no such tool exists, so a Goose attach proposes Behaviors without being able to act through ai-buddy's tools. What the probe hands over is the stdio shim, since the loopback server starts with the app and not with `--probe-harness`, so the http server Goose advertises is untested here. A user-configured Goose extension is untested too. |
