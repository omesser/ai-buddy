...
| `goose` | `goose acp` | First-party, Block. `goose` alone is the interactive CLI, so the subcommand is the whole of the row. A probe of goose 1.52.0 with no provider exits 2: `session/new` returns Internal error because `GOOSE_PROVIDER` is unset (`goose configure`). A completed turn is not claimed. |
...
| `goose` | unverified | unverified | unverified | unverified | unverified |
