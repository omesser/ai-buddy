...
mod tests {
    ...
    /// Black Mage Character Package loads with all required animations.
    #[test]
    fn black_mage_character_package_loads() {
        let mut package: PackageBytes = BTreeMap::new();

        // Load the Black Mage package from the characters directory
        let base_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .unwrap()
            .parent()
            .unwrap()
            .join("characters/black-mage");

        // Read character.manifest
        let manifest_path = base_path.join("character.manifest");
        let manifest =
            std::fs::read(&manifest_path).expect("Black Mage character.manifest should exist");
        package.insert("character.manifest".to_string(), manifest);

        // Read personality.txt if it exists
        let personality_path = base_path.join("personality.txt");
        if personality_path.exists() {
            let personality = std::fs::read(&personality_path).unwrap();
            package.insert("personality.txt".to_string(), personality);
        }

        // Read all frame files from frames/ directory
        let frames_dir = base_path.join("frames");
        if frames_dir.exists() {
            for entry in std::fs::read_dir(&frames_dir).unwrap() {
                let entry = entry.unwrap();
                let path = entry.path();
                if path.extension().and_then(|s| s.to_str()) == Some("png") {
                    let frame_name = format!("frames/{}", entry.file_name().to_string_lossy());
                    let frame_data = std::fs::read(&path).unwrap();
                    package.insert(frame_name, frame_data);
                }
            }
        }

        let character = load(&package).expect("Black Mage package should load successfully");

        assert_eq!(character.name, "Black Mage");

        // Verify all required animations are present
        for required in &REQUIRED_ANIMATIONS {
            assert!(
                character.animations.contains_key(*required),
                "Black Mage should declare required animation: {}",
                required
            );
        }

        // Verify personality is present and within limit
        assert!(
            !character.personality.is_empty(),
            "Black Mage should have a personality"
        );
        assert!(character.personality.len() <= PERSONALITY_LIMIT);

        // Verify at least one behavior is declared
        assert!(
            !character.behaviors.is_empty(),
            "Black Mage should have behaviors"
        );
    }
